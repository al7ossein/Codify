...
def send_verification_email(email, code):
    ...
    try:
        msg = Message('تحقق من بريدك الإلكتروني - Codify',
                     ...
                     recipients=[email])
        msg.body = f'''مرحباً بك في Codify!
...
def get_profile(current_user):
    try:
        ...
        quiz_data = [{
            'course': q.course_rel.title,
            'track': q.course_rel.track,
            'score': q.score,
            'passed': q.passed,
...
def get_leaderboard():
    try:
        # نحصل على أفضل 10 مستخدمين بناءً على عدد الاختبارات التي اجتازوها
        top_users = db.session.query(
            User,
            db.func.count(QuizResult.id).label('passed_quizzes')
        ).join(QuizResult).filter(
            QuizResult.passed == True
        ).group_by(User.id).order_by(
            db.desc('passed_quizzes')
        ).limit(10).all()
        ...
        leaderboard = [
            ...
            for user, passed_count in top_users
        ]
